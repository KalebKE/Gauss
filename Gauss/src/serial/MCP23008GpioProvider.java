 ...
 * 
 */
public class MCP23008GpioProvider extends GpioProviderBase implements
		GpioProvider
{
	...
	public void setMode(Pin pin, PinMode mode)
	{
		...
		{
			throw new InvalidPinModeException(pin, "Invalid pin mode ["
					+ mode.getName() + "]; pin [" + pin.getName()
		...
		{
			// shutdown and destroy monitoring thread since there are no input
			// pins configured
			if (monitor != null)
			{
				monitor.shutdown();
	...
	public void setState(Pin pin, PinState state)
	{
		...
		{
			throw new InvalidPinModeException(pin, "Invalid pin mode on pin ["
					+ pin.getName() + "]; cannot setState() when pin mode is ["
	...
	public PinState getState(Pin pin)
	{
		...
		// determine pin state
		result = (currentStates & pinAddress) == pinAddress ? PinState.HIGH
				: PinState.LOW;
	...
	private class GpioStateMonitor extends Thread
	{
		...
		public void run()
		{
			while (!shuttingDown)
			{
				try
				{
					// only process for interrupts if a pin is configured as an
					// input pin
					if (currentDirection > 0)
					{
						// process interrupts
						...
						if (pinInterrupt > 0)
						{
							...
							for (Pin pin : MCP23008Pin.ALL)
							{
								...
								{
									// System.out.println("INTERRUPT ON PIN [" +
									// pin.getName() + "]");
		...
		private void dispatchPinChangeEvent(int pinAddress, PinState state)
		{
			...
			for (Pin pin : listeners.keySet())
			{
				...
				if (pin.getAddress() == pinAddress)
				{
					...
					{
						listener.handlePinEvent(new PinDigitalStateChangeEvent(
								this, pin, state));
					}
				}
			}
